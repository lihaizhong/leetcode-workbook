...
function deserializer(list: number[] = []): ListNode | null {
...
describe("两数相加", () => {
  it("[9,9,9,9,9,9,9], [9,9,9,9]", () => {
    const l1: ListNode | null = deserializer([9, 9, 9, 9, 9, 9, 9]);
    const l2: ListNode | null = deserializer([9, 9, 9, 9]);

    expect(serializer(l1, l2)).toEqual([8, 9, 9, 9, 0, 0, 0, 1]);
  });

  it("[0], [0]", () => {
    const l1: ListNode | null = deserializer([0]);
    const l2: ListNode | null = deserializer([0]);

    expect(serializer(l1, l2)).toEqual([0]);
  });

  it("[2,4,3], [5,6,4]", () => {
    const l1: ListNode | null = deserializer([2, 4, 3]);
    const l2: ListNode | null = deserializer([5, 6, 4]);
